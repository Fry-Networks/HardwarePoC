...
$makeProfile = $null
if ($toolsDir) { $makeProfile = Join-Path $toolsDir 'make_profile.py' }
if ($makeProfile -and (Test-Path $makeProfile)) {
  & py $makeProfile --code $Code --version $Version --version-check-sec $VersionCheckSec
} else {
  Write-Warning "make_profile.py not found; skipping profile generation step."
}
...
if ($Use1Password) {
  ...
  try {
    ...
    $bearerToken = if ($OPRefBearerToken) { (& op read $OPRefBearerToken).Trim() } else { $null }
    ...
    if ($bearerToken) { 
      $cfg.external_api.bearer_token = $bearerToken
    ...
    if ($OPRefSigningKey) {
      ...
      $cfg.local_signing_key_hex = $signKey; Write-Host ("Signing key OK (fingerprint {0})" -f (Convert-SecretKeyToMasked $signKey))
    }
    if ($OPRefUpdateRepo) {
  ...
  if (-not $makeEncrypted -or -not (Test-Path $makeEncrypted)) { throw "make_encrypted_config.py not found in tools directory; cannot embed 1Password secrets." }
  $enc = & py $makeEncrypted --in $tmpCfg --json; if ($LASTEXITCODE -ne 0) { throw "Failed to run make_encrypted_config.py" }
...
$svcArgs = @('--clean','--onefile','--noconsole','--noconfirm','--name', $SvcName, '--collect-binaries','h3','--collect-all','shapely','--collect-all','geoip2','--distpath', $svcDistDir)
 ...
 # ----- Icon resolution & conversion (supports .ico, .png, .jpg) -----
  $iconBase = switch ($Code.ToUpper()) { 'BM'{'BM'} 'IDM'{'DB'} 'ODM'{'DB'} 'ISM'{'GNSS'} 'OSM'{'GNSS'} 'RDN'{'NODE'} 'SVN'{'NODE'} 'SDN'{'NODE'} 'IRM' {'RAD'} default{'frynetworks_logo'} }
