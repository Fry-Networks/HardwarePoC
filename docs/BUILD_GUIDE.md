...
The executable name format is: `FRY_PoC_{MINER_CODE}_v{VERSION}`
...
pyinstaller --onefile --name FRY_PoC_ISM_v5.5.5 miner_online_simple.py

# For Bandwidth Miner (BM)  
pyinstaller --onefile --name FRY_PoC_BM_v5.5.5 miner_online_simple.py

# For AI Edge Miner (AEM)
pyinstaller --onefile --name FRY_PoC_AEM_v5.5.5 miner_online_simple.py
```

Or for Windows with icon:
```bash
pyinstaller --onefile --windowed --icon=icon.ico --name FRY_PoC_ISM_v5.5.5 miner_online_simple.py
```
...
- 📦 **Branded Executable** - FRY_PoC_{MINER_CODE}_v{VERSION} for specific miner type
...
├── FRY_PoC_ISM_v5.5.5.exe         # ISM Branded Miner executable (versioned)
└── miner_config.enc           # Encrypted miner configuration (installer-created)

# Bandwidth Miner (BM)
C:\Program Files\FryNetworks\BM Miner\
├── FRY_PoC_BM_v5.5.5.exe          # BM Branded Miner executable (versioned)
└── miner_config.enc           # Encrypted miner configuration (installer-created)
...
├── FRY_PoC_ISM_v5.5.5             # ISM Branded Miner executable (versioned)
└── miner_config.enc          # Encrypted miner configuration (installer-created)  

# AI Edge Miner (AEM)
/opt/frynetworks/aem-miner/
├── FRY_PoC_AEM_v5.5.5             # AEM Branded Miner executable (versioned)
└── miner_config.enc          # Encrypted miner configuration (installer-created)  
...
2. ✅ The executable should run without requiring any config files except `miner_config.enc`
...
# 7. Distribute
# Distribute: FRY_PoC_ISM_v5.5.5.exe (ISM miner executable with version)
# For other miner types: FRY_PoC_BM_v5.5.5.exe, FRY_PoC_AEM_v5.5.5.exe, etc.
