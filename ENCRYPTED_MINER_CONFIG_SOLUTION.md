...
cd dist && pyinstaller --onefile --name FRY_BM_v5.5.4 miner_online_simple.py
```
**Result**: `FRY_BM_v5.5.4.exe` - ISM branded executable for all deployments
...
- Falls back to `minerkey.txt` if encrypted config not found
- Zero configuration needed

## File Structure
```
Deployment Directory:
├── FRY_BM_v5.5.4.exe     # ISM Branded Miner executable (versioned)
├── miner_config.enc      # Encrypted miner configuration (unique per deployment)
└── minerkey.txt          # Fallback (optional)
```
